...
function maxNonAdjacentSum(nums) {
  ...
  for (let i = 1; i < nums.length; i++) {
    // if (table[i] < nums[i]) table[i] = nums[i];
    let skipLeftNeighor = table[i - 2] === undefined ? 0 : table[i - 2];
    let includeNum = skipLeftNeighor + nums[i];
...
//   let firstEl = nums[0];
//   memo[nums.length] = Math.max(firstEl + maxNonAdjacentSum(nums.slice(2), memo),
